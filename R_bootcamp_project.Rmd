...
plot(UNIDIR ~ air_temperature, data = join_zaehlung_wetter,
     col = FK_ZAEHLER,
     pch = 19,
     main = "Count of pedestrians and bikers vs air temperature")
##
legend("topleft",
       pch = 19,
       legend = c("fussgaenger","velo"),
       col = c("black", "red"))
```
```{r}
...
```{r}
plot(UNIDIR ~ air_temperature, data = join_zaehlung_wetter,
     main = "Model 'lm.join_zaehlung_wetter2'",
     col = FK_ZAEHLER)
##
legend("topleft", 
       pch = 19,
...
#coef(lm.join_zaehlung_wetter2["(Intercept)"])
#coef(lm.join_zaehlung_wetter2["(Intercept)"] + coef(lm.join_zaehlung_wetter2)["FK_ZAEHLERvelo"])
```
```{r}
lm.join_zaehlung_wetter3 <- lm(UNIDIR ~ air_temperature * FK_ZAEHLER, data = join_zaehlung_wetter)
plot(UNIDIR ~ air_temperature, data = join_zaehlung_wetter,
     main = "Model 'lm.join_zaehlung_wetter3'",
     xlim = c(-5, 40), 
     ylim = c(-5, 1200),
     col = FK_ZAEHLER)
##
...
```{r}
interactive_graph <- plot_ly(data.frame(join_zaehlung_wetter), x = ~UNIDIR, y = ~air_temperature, z = ~precipitation, color = factor(join_zaehlung_wetter$TAGESZEIT), alpha = 0.7) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'UNIDIR'),
                      yaxis = list(title = 'Air temperature'),
                      zaxis = list(title = 'Precipitation')))
