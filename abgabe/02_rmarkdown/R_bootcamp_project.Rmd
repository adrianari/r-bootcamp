...
```{r}
lm.join_zaehlung_wetter <- lm(UNIDIR ~ air_temperature + precipitation, data = join_zaehlung_wetter)
summary(lm.join_zaehlung_wetter)
```

Interpretation: P-Value is very small, there is high that it is not 0 for air temperature and precipitation. If there is a count of zero pedestrians and bikers per hour, the temperature would be expected to be at 5.8 degrees.This does not make much sense, as there are other factors involved.
...
ggplot(join_zaehlung_wetter, aes(x = air_temperature, y = UNIDIR)) +
  ...
  geom_segment(aes(xend = air_temperature, yend = predicted), alpha = 0.05) +
  geom_point(aes(color = abs(residuals), size = abs(residuals))) +
  scale_color_continuous(low = "green", high = "red") +
  guides(color = FALSE, size = FALSE) +
  geom_point(aes(y = predicted), shape = 1) +
  labs(title = "Residuals", x = "Air temperature") +
  theme_bw()
```
