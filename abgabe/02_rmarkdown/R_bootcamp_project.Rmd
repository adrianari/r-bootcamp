...
Third, we categorise the hour indications into categories of the same length (6 hour indications per factor) "morgen", "vormittag", "nachmittag" and "abend". 
...
```{r}
histogram(~ air_temperature, data=join_zaehlung_wetter, xlab = "Air temperature [°C]", main="Distribution of air temperature", breaks = 20)
```
...
```
In the boxplot it can be seen, that the median of the count of bikers is slightly higher than the count of pedestrians.  
...
There is strong evidence, that the mean growth rate for air temperature is not eual to zero. If there is a count of zero pedestrians and bikers per hour, the temperature would be expected to be at 5.8 degrees.This does not make much sense, as there are other factors involved.
...
# Train the model
#Cross validation for linear regression
cv.model <- train(UNIDIR ~ air_temperature, data = join_zaehlung_wetter, method = "lm",
               trControl = train.control)
#cross validation for gam model
cv.gam.model <- train(UNIDIR ~ air_temperature + precipitation, data = join_zaehlung_wetter, method = "gam",
...
Such questions would surely be interesting to be analysed in a further paper. One could also specially take into account days, where there are big days in Zurich like Street Parade. 