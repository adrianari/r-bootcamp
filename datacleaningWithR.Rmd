...
The following paper looks at the influence of the weather on the number of pedestrians and bikers in Zurich. More specifically, the focus lies on the place of Mythenquai. Mythenquai has been chosen ouf of the reason that both weather measurements as well as data regarding the number of pedestrians / bikers are 

For this, two different data sets


*Datasets*
**Dataset 1**
2019_verkehrszaehlungen_werte_fussgaenger_velo
...
First, the required libraries are set up:
```{r}
# 1.1.1 Libraries
library(dplyr)
library(stringr)
...
We further check if we are in the correct working directory:
```{r}
...
Discussion: very reliant on the quality of the data. (We do not know exactly how the counting stations worked)